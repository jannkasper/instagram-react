...
export const loadUserStories = async (req, res) => {
    ...
    const encodedParamsURL = convertPathParams({user_id: userId, include_chaining: true, include_reel: true, include_suggested_users: false, include_logged_out_extras: false, include_highlight_reels: true, include_live_status: true });

    const graphql = await instagramFetch.get(STORIES_PATH + encodedParamsURL)
        .then(getGraphql)
        .catch(errorHandling);
